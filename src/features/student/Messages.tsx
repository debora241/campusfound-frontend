...
import { STUDENT_MESSAGES } from "./data";

export function Messages() {
  const [activeId, setActiveId] = useState(STUDENT_MESSAGES[0].id);
  const active = STUDENT_MESSAGES.find((m) => m.id === activeId)!;
  const { register, handleSubmit, reset } = useForm<{ reply: string }>();

  return (
    <div>
      <PageHeader title="Messages" description="Conversations with teachers and school administration" />
